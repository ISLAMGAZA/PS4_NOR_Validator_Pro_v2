"""
Firmware version database for PS4 NOR analysis.
Matches Wee Tools data structure exactly.
Maps MD5 hashes of EMC_IPL, EAP_KBL, and Torus partitions
to { 't': type_code, 'fw': [version_list] }.
"""
...
'e539774a1f6b55ef74aab9017279c64e' : { 't':0x0D, 'fw':['10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
...
'c1aa7f63ba7aac8c8695970a416e1342' : { 't':0x20, 'fw':['10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
...
'07f41f9ac4370ce7736885ed5fa4372c' : { 't':0x24, 'fw':['10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
...
'78bcc7e6fcafd9a5de8c32d5bf802d09' : { 't':0x2A, 'fw':['10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
}
...
'147322f4fbcf622e688a0891a07199ef' : { 't':0x0E, 'fw':['13.52']},
...
'6039b007ad4f09225a23f95fb3bddaec' : { 't':0x21, 'fw':['13.52']},
...
'864e8d7e9693c09070c415c296cfdbc9' : { 't':0x25, 'fw':['13.52']},
...
'b9e515dad66a796d0b76bb625eb748c5' : { 't':0x2B, 'fw':['13.52']},
}
...
'e84fb3af2096d22c7d7b7138ff292b1e' : { 't':0x03, 'fw':['9.50', '9.51', '9.60', '10.00', '10.01', '10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
...
'50b0085e8917ffca236bb449a81fd3eb' : { 't':0x22, 'fw':['9.50', '9.51', '9.60', '10.00', '10.01', '10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
...
'c23090c0bdce1bcb0f6bada5ef340e4e' : { 't':0x30, 'fw':['9.50', '9.51', '9.60', '10.00', '10.01', '10.50', '10.70', '10.71', '11.00', '11.02', '11.50', '11.52', '12.00', '12.02', '12.50', '12.52', '13.00', '13.02', '13.04', '13.50', '13.52']},
}
...
SWITCH_TYPES = ['Off', 'Fat 10xx/11xx', 'Fat/Slim/PRO 12xx/2xxx/7xxx', 'General', 'Extra']
...
def is_fw_in_list(fw, fw_list):
    fw_int = fw_to_int(fw)
    min_fw = fw_to_int(fw_list[0])
    max_fw = fw_to_int(fw_list[-1])
    return min_fw <= fw_int <= max_fw


def get_fw_range(md5_hex, db=EMC_IPL_MD5):
    if md5_hex in db:
        fw_list = db[md5_hex]['fw']
        return fw_list[0], fw_list[-1]
    return None, None


def ranges_are_distinct(range_a, range_b):
    min_a, max_a = range_a
    min_b, max_b = range_b
    if min_a is None or max_a is None or min_b is None or max_b is None:
        return None
    if fw_to_int(max_a) < fw_to_int(min_b):
        return "A_lower"
    if fw_to_int(max_b) < fw_to_int(min_a):
        return "B_lower"
    return None


def detect_southbridge(emc_md5, eap_md5):
    eap_info = EAP_KBL_MD5.get(eap_md5)
    emc_info = EMC_IPL_MD5.get(emc_md5)
    eap_t = eap_info['t'] if eap_info else 0
    emc_t = emc_info['t'] if emc_info else 0
    for sb in SOUTHBRIDGES:
        if eap_t in sb['code']:
            return sb
        if emc_t in sb['code']:
            return sb
    return {'code': [emc_t, eap_t], 'name': 'Unknown', 'ic': 'XX'}


def get_slot_switch_info(data):
    pattern = list(data[0x201000:0x201010])
    for i, blob in enumerate(SWITCH_BLOBS):
        if blob['v'] == pattern:
            return SWITCH_TYPES[blob['t']] + ' [#' + str(i + 1) + ']'
    return SWITCH_TYPES[0]
